...
@endphp
  ...
  <style>
    :root{ --brand:{{ $brand }}; --brand-d:{{ $brandD }}; --brand-soft:{{ $brandSoft }}; --btn:{{ $btn }}; --btn-shadow:{{ $btnShadow }}; --bg-pearl:#f4f7fa; }
    ...
    .text-brand{color:var(--brand)!important}.bg-brand{background:var(--brand)!important}.bg-brand-soft{background:var(--brand-soft)!important}
    ...
    .pb-nav{ padding-bottom:calc(env(safe-area-inset-bottom) + 6rem) }
  </style>
</head>
...
<div x-data="vApp()" class="w-full max-w-md flex flex-col gap-6 pb-nav">
  @include('verify._intel-alerts')

  {{-- ✨ Redesigned logo / brand header --}}
  <header class="glass-panel p-5 relative overflow-hidden">
    <div class="absolute -top-10 -right-8 w-28 h-28 rounded-full grad-brand opacity-10"></div>
    <div class="relative flex items-center gap-4">
      <div class="w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 overflow-hidden text-brand">
        @if($cfg['logo_path'])
          <img src="{{ asset('storage/'.$cfg['logo_path']) }}" alt="" class="w-full h-full object-contain">
        @else
          <i class="fa-solid fa-shield-halved text-3xl"></i>
        @endif
      </div>
      <div class="flex-1 min-w-0">
        <h1 class="font-extrabold text-xl tracking-tight leading-none truncate">{{ $cfg['brand_name'] }}</h1>
        <p class="text-[11px] font-bold uppercase tracking-[0.18em] text-brand mt-1.5"><i class="fa-solid fa-circle-check mr-1"></i>Secure Authenticity</p>
      </div>
      @if($cfg['report_form'])
      <button @click="openSheet()" class="w-10 h-10 rounded-full bg-slate-50 border border-slate-100 flex items-center justify-center text-slate-400 hover:text-rose-500 transition shrink-0" aria-label="Report">
...
@else
  ...
  @php
    ...
    $eventIcons = ['scanned'=>'fa-qrcode','units_generated'=>'fa-plus','units_removed'=>'fa-minus','printed'=>'fa-print','packed'=>'fa-box','dispatched'=>'fa-truck-fast','received'=>'fa-warehouse'];
    $rowMap = [
    ...
    <div x-show="tab==='history'" x-cloak class="fade-view space-y-6">
      ...
      <div>
        <div class="flex items-center justify-between mb-3">
          <p class="text-[10px] font-bold uppercase tracking-widest text-slate-400">Scan History</p>
          @if($verifications->count())<span class="text-[10px] font-bold text-brand bg-brand-soft px-2 py-0.5 rounded-full">{{ $verifications->count() }}</span>@endif
        </div>
        <div class="space-y-2">
          @forelse($verifications as $v)
            @php
              $chips = [];
              if($cfg['show_country'] && $v->country) $chips[] = ['fa-earth-americas', $v->country];
              if($cfg['show_city'] && $v->city) $chips[] = ['fa-location-dot', $v->city];
              if($cfg['show_ip'] && $v->ip_address) $chips[] = ['fa-network-wired', $v->ip_address];
              if($cfg['show_device']){ $d = trim(implode(' ', array_filter([ucfirst($v->device_type ?? ''), $v->browser]))); if($d) $chips[] = ['fa-mobile-screen', $d]; }
            @endphp
            <div class="bg-slate-50 rounded-2xl border border-slate-100 p-3">
              <div class="flex items-center gap-3">
                <div class="w-9 h-9 rounded-xl bg-brand-soft text-brand flex items-center justify-center shrink-0"><i class="fa-solid fa-qrcode text-sm"></i></div>
                <div class="flex-1 min-w-0">
                  <p class="text-sm font-bold text-slate-700 truncate">Scanned / Verified</p>
                  <p class="text-[11px] text-slate-400">{{ $v->created_at?->format('d M Y, H:i') }} · Public</p>
                </div>
                @if($v->is_proxy)<span class="text-[9px] font-bold text-amber-600 bg-amber-50 border border-amber-100 px-2 py-0.5 rounded-full shrink-0">VPN</span>@endif
              </div>
              @if(count($chips))
                <div class="flex flex-wrap gap-1.5 mt-2.5 pl-12">
                  @foreach($chips as $c)
                    <span class="inline-flex items-center gap-1 text-[10px] font-semibold text-slate-500 bg-white border border-slate-100 px-2 py-0.5 rounded-full">
                      <i class="fa-solid {{ $c[0] }} text-[9px] text-brand"></i>{{ $c[1] }}
                    </span>
                  @endforeach
                </div>
              @endif
          ...
          @empty
            <p class="text-sm text-slate-400 text-center py-3">No scans yet.</p>
          @endforelse
        </div>
      </div>
    </div>
...
@endif
  ...
  @if($cfg['footer'])<p class="text-center text-[11px]" style="color:{{ $footerCol }}">{{ $cfg['footer'] }}</p>@endif

@if(!$block)
  {{-- ═══════════════ FLOATING FOOTER TAB BAR ═══════════════ --}}
  <nav class="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-md px-4 pb-4 z-30" style="padding-bottom:calc(env(safe-area-inset-bottom) + 1rem)">
    <div class="glass-panel flex items-stretch gap-1 p-2" style="border-radius:24px">
      @php $tabs5 = [['info','Info','fa-circle-info'],['images','Images','fa-camera'],['history','History','fa-clock-rotate-left']]; @endphp
      @foreach($tabs5 as $t)
        <button @click="tab='{{ $t[0] }}'" class="flex-1 flex flex-col items-center gap-1 py-2.5 rounded-2xl transition"
                :class="tab==='{{ $t[0] }}' ? 'bg-brand-soft text-brand' : 'text-slate-400'">
          <i class="fa-solid {{ $t[2] }} text-lg"></i>
          <span class="text-[10px] font-bold uppercase tracking-wider">{{ $t[1] }}</span>
        </button>
      @endforeach
    </div>
  </nav>
