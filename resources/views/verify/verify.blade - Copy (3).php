...
@if($block)
  {{-- ═══════════════ BLOCKED / RESTRICTED ═══════════════ --}}
  @php
    // Severity tone + icon per reason (semantic — fixed). Title/text are admin-editable.
    $style = [
...
@else
  ...
  @php
    ...
    // Resolver for the configurable highlight card (left/right slots).
      ...
      <template x-for="t in tabs" :key="t.k">
        <button @click="tab=t.k" class="flex-1 flex flex-col items-center gap-1 py-1 transition group focus:outline-none">
          <span class="flex items-center justify-center w-14 h-9 rounded-2xl transition duration-200"
                :class="tab===t.k ? 'bg-brand-soft text-brand scale-100' : 'text-slate-400 group-active:bg-slate-100'">
