...
<head>
  ...
  <style>
    ...
    .shimmer{background:linear-gradient(110deg,rgba(255,255,255,0) 30%,rgba(255,255,255,.5) 50%,rgba(255,255,255,0) 70%);background-size:200% 100%;animation:shimmer 2.6s infinite}
...
<style>
  ...
  .grad-brand{background:linear-gradient(150deg,var(--brand),var(--brand-d))!important}
...
@if($block)
  ...
  @php
    ...
    $grad = ['rose'=>'from-rose-500 to-red-600','amber'=>'from-amber-500 to-orange-500','slate'=>'from-slate-600 to-slate-800'][$style[1]];
  @endphp
  <div class="min-h-screen flex flex-col">
    <div class="relative bg-gradient-to-b {{ $grad }} text-white px-6 pt-12 pb-20 text-center rounded-b-[2.5rem] overflow-hidden">
      <div class="absolute -top-10 -right-10 w-40 h-40 rounded-full bg-white/10"></div>
      <div class="absolute -bottom-12 -left-8 w-36 h-36 rounded-full bg-black/10"></div>
      <div class="relative">
        <div class="animate-pop w-24 h-24 mx-auto mb-6 rounded-full bg-white/15 backdrop-blur flex items-center justify-center ring-4 ring-white/25">
        ...
        <p class="text-xs font-bold uppercase tracking-[0.2em] text-white/70">{{ $style[0] }}</p>
        <h1 class="text-2xl font-black mt-1">{{ $m['title'] }}</h1>
      </div>
    </div>
    <div class="flex-1 px-6 -mt-10">
      <div class="bg-white rounded-3xl shadow-xl ring-1 ring-slate-100 p-6 rise">
        <p class="text-[15px] leading-relaxed text-slate-600 text-center">{{ $m['text'] }}</p>
...
@else
  ...
  <div class="pb-28">
    {{-- ✨ Beautiful header --}}
    <div class="relative grad-brand text-white px-6 pt-10 pb-24 rounded-b-[2.75rem] overflow-hidden">
      <div class="absolute -top-16 -right-12 w-48 h-48 rounded-full bg-white/10 blur-2xl floaty"></div>
      <div class="absolute top-10 -left-16 w-44 h-44 rounded-full bg-black/10 blur-2xl"></div>
      <div class="absolute bottom-0 right-8 w-24 h-24 rounded-full bg-white/10 blur-xl floaty"></div>
      ...
      <div class="relative">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-2 opacity-95">
            ...
            @else
              <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24"><path d="M12 1l9 4v6c0 5.25-3.6 9.74-9 11-5.4-1.26-9-5.75-9-11V5l9-4z"/></svg>
            @endif
            <span class="font-extrabold tracking-tight">{{ $cfg['brand_name'] }}</span>
          </div>
          @if($cfg['report_form'])
          <button @click="openSheet()" class="w-9 h-9 rounded-full bg-white/15 backdrop-blur flex items-center justify-center text-white/90 active:scale-95 transition" aria-label="Report">
        ...
        <div class="mt-7 flex flex-col items-center text-center">
          <div class="relative mb-4">
            <div class="absolute inset-0 rounded-[1.75rem] bg-white/20 blur-md"></div>
            @if($cfg['show_product_photo'] && $images->first())
              <div class="relative w-28 h-28 rounded-[1.75rem] overflow-hidden ring-4 ring-white/40 shadow-2xl animate-pop bg-white">
                <img src="{{ $images->first()->url }}" class="w-full h-full object-cover" alt="{{ $product?->name }}">
              </div>
              <span class="absolute -bottom-2 -right-2 w-9 h-9 rounded-full bg-white text-brand flex items-center justify-center ring-4 ring-white/30 shadow-lg">
            ...
            @else
              <div class="relative w-24 h-24 rounded-full bg-white/15 backdrop-blur flex items-center justify-center ring-4 ring-white/30 animate-pop">
                <svg class="w-12 h-12" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6 6 9-13.5"/></svg>
              </div>
            @endif
          </div>

          <span class="relative overflow-hidden inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/20 backdrop-blur text-[11px] font-bold uppercase tracking-[0.18em] text-white">
            <span class="absolute inset-0 shimmer"></span>
            <svg class="w-3.5 h-3.5 relative" fill="currentColor" viewBox="0 0 24 24"><path d="M12 1l9 4v6c0 5.25-3.6 9.74-9 11-5.4-1.26-9-5.75-9-11V5l9-4z"/></svg>
            <span class="relative">{{ $cfg['genuine_title'] }}</span>
          </span>
          <h1 class="text-[26px] leading-tight font-black mt-3 rise">{{ $product?->name ?? 'Genuine Product' }}</h1>
          ...
          <button @click="confirmVerify()" :disabled="verified"
                  class="mt-5 inline-flex items-center gap-2 px-7 py-3 rounded-2xl bg-white text-btn font-extrabold text-sm shadow-xl active:scale-[.97] transition rise disabled:opacity-95">
    ...
    <div class="px-5 -mt-12">
      <div class="bg-white rounded-3xl shadow-xl ring-1 ring-slate-100 p-4 flex items-center gap-4 rise">
        ...
        @if($images->count())
          <div class="grid grid-cols-2 gap-3">
            @foreach($images as $img)
              <div class="rounded-3xl overflow-hidden ring-1 ring-slate-100 bg-white aspect-square flex items-center justify-center p-1">
    ...
    @if($cfg['footer'])<p class="text-center text-xs px-6 mt-6" style="color:{{ $footerCol }}">{{ $cfg['footer'] }}</p>@endif
  </div>
